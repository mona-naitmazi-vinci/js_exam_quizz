...
const getRandomQuestions = () => {
  // min id = 1 et max id = 30
  const id1 = Math.floor(Math.random() * (30 - 1 + 1) + 1);
  const id2 = Math.floor(Math.random() * (30 - 1 + 1) + 1);
  const id3 = Math.floor(Math.random() * (30 - 1 + 1) + 1);

  const tabQuestions = [questions[id1 - 1], questions[id2 - 1], questions[id3 - 1]];
  return tabQuestions;
}
